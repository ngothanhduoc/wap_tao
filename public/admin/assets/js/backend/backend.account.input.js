...
var BACKEND = {
           ...
           $('#frm-add-account').submit(function(e) {
			...
			console.log(form);
                        if($('#jqxDropdownlistGroup').val() != ''){
                            form.append("id_group", arrGroupName[$('#jqxDropdownlistGroup').val()]);
                        }
			$.ajax({
				...
				if (data.code == 0) {
					...
					if (m.username!= "") {
						...
						$('body,html').animate({scrollTop: c}, 800);
                                        ...
                                        if (m.id_group != "") {
						$('#id_group').html(m.id_group);
						var c = $("#id_group").position().top;
						$('body,html').animate({scrollTop: c}, 800);
					}
				}
    ...
    init: function() {
        BACKEND.commit();
        BACKEND.initDropdownlistGroup(GROUP_NAME);
    ...
    initDropdownlistGroup: function(data) {
		...
		$("#jqxDropdownlistGroup").jqxDropDownList({source: source, /*selectedIndex: 0,*/ width: '25%', height: '25', theme: 'office', placeHolder: "Vui lòng chọn"});
