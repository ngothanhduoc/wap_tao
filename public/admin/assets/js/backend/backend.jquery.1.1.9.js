...
function openKCFinderFixSize(field, width, height) {
    window.KCFinder = {
        callBack: function(url) {
            ...
            img.onload = function() {
                if (this.width == width && this.height == height) {
                    ...
                    alert('Hình chưa đúng kích thước quy định, vui lòng chọn hình khác');
                }
            };
            window.KCFinder = null;
        }
...
function checkLimitString(element, minString, maxString) {
    var name = (($(element).attr('id') != null && $(element).attr('id') != undefined && $(element).attr('id') != '') ? $(element).attr('id') : $(element).attr('class'));
    if (name == null || name == undefined)
        return false;
    if (minString == 0)
        $(element).after('<span class="' + name + '_len" style="color:red; font-weight:bold;position:absolute;"> * Còn ' + (maxString - $(element).val().length) + ' ký tự</span>');
    else
        $(element).after('<span class="' + name + '_len" style="color:red; font-weight:bold;position:absolute;"> * ' + $(element).val().length + ' ký tự trong khoảng ' + minString + '-' + maxString + ' ký tự</span>');
    $(element).keyup(function() {
        var len = $(element).val().length;
        $('button:submit').attr("disabled", false);
        if (minString == 0) {
            $('.' + name + '_len').html(' * Còn ' + (maxString - len) + ' ký tự');
            if (len > maxString) {
                $('button:submit').attr("disabled", true);
                $('.' + name + '_len').html(' * Bạn nhập quá số lượng ký tự quy định!');
            }
        } else {
            $('.' + name + '_len').html(' * ' + $(element).val().length + ' ký tự trong khoảng ' + minString + '-' + maxString + ' ký tự');
            if (len < minString || len > maxString) {
                $('button:submit').attr("disabled", true);
                $('.' + name + '_len').html(' * Bạn nhập không đúng số lượng ký tự quy định!');
            }
        }
    });
}