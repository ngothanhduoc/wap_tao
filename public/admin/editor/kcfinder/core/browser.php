...
class browser extends uploader {
    ...
    protected function act_reduceImage() {
        $message = '';
        if (isset($this->post['data'])) {

            //$dir = str_replace('../../../', '', $this->postDir());
            $dir = $this->postDir();
            $data = $this->post['data'];
            $arr = explode('***', $data);
            for ($i = 0; $i < count($arr); $i++) {
                ...
                file_put_contents($dir . '/1_' . $arr[$i], $compressed_png_content);
            }
        }
    ...
    protected function compress_png($path_to_png_file, $max_quality = 90) {
        if (!file_exists($path_to_png_file)) {
            return "File does not exist: $path_to_png_file";
        }
