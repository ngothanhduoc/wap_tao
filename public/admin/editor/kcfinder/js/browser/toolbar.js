...
browser.initToolbar = function() {
    ...
    $('#toolbar a[href="kcact:reduce"]').click(function() {
        browser.reduce();
        return false;
    });
...
browser.uploadFile = function(form) {
    ...
    $('#uploadResponse').load(function() {
        ...
        // Optimize PNG Images
        var str = '';
        var dir = browser.uploadURL + '/' + browser.dir;
        ...
        setTimeout(function() {
            $.ajax({
                dataType: 'json',
                url: '/backend/ajax/reduceImage',
                async: true,
                type: 'post',
                data: {'data': str, 'dir': dir},
                success: function(data) {
                    browser.refresh();
                },
                error: function() {
...
browser.reduce = function() {
    this.fadeFiles();
    var dir = this.uploadURL + '/' + this.dir;
    var files = $('.file.selected').find('.name').get();
    var str = '';
    for (var i = 0; i < files.length; i++) {
        if (i == 0) {
            str += files[i].textContent;
        } else {
            str += '***' + files[i].textContent;
        }
    }
