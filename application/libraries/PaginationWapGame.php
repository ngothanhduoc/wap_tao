...
class PaginationWapGame{


	public function paging($count,$per_page,$cur_page,$nph,$sort)
	{
           if($nph != 0){
               $n = '&nph='.$nph;
           }else{
           ...
           $paging = '';
            if (($count / $per_page) > 1) {
                ...
                if ($first_btn && $cur_page > 1) {
                    $paging .= "<span p='1' class='inactive first'><a href='" . base_url() . "mobile-game.moi?trang=1".$n.$s."'>&nbsp;</a></span>";
                } else if ($first_btn) {
                    $paging .= "<span p='1' class='inactive first'><a href='" . base_url() . "mobile-game.moi?trang=1".$n.$s."'>&nbsp;</a></span>";
                }
                ...
                for ($i = $start_loop; $i <= $end_loop; $i++) {
                    if ($cur_page == $i) {
                        ...
                        $paging .= "<span p='$i' class='inactive'><a href='" . base_url() . "mobile-game.moi?trang=" . $i . $n . $s . "'>" . $i . "</a></span>";
                    }
                }
                ...
                if ($last_btn && $cur_page < $no_of_paginations) {
                    $paging .= "<span p='" . $no_of_paginations . "' class='inactive last'><a href='" .  base_url() . "mobile-game.moi?trang=" . $no_of_paginations . $n . $s . "'>&nbsp;</a></span>";
                } else if ($last_btn) {
                    $paging .= "<span p='$no_of_paginations' class='inactive last'><a href='" . base_url() . "mobile-game.moi?trang=" . $no_of_paginations . $n . $s . "'>&nbsp;</a></span>";
                }
                $paging .= "</div>";
