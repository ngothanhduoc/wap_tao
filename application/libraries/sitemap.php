...
class sitemap {
     ...
     */
    public function createSitemap($content = "") {
        if (!isset($this->urls))
        ...
        foreach (array_chunk($this->urls, $this->maxURLsPerSitemap) as $sitemap) {
            ...
            foreach ($sitemap as $url) {
                if (!$this->isFirst) {
                    ...
                    $row = $dom->ownerDocument->createElement('url');
                    $loc = $row->ownerDocument->createElement('loc', htmlspecialchars($url['loc']));
                    $row->appendChild($loc);
                    if (isset($url['lastmod'])) {
                        $lastmod = $row->ownerDocument->createElement('lastmod', $url['lastmod']);
                        $row->appendChild($lastmod);
                    }
                    if (isset($url['changefreq'])) {
                        $changefreq = $row->ownerDocument->createElement('changefreq', $url['changefreq']);
                        $row->appendChild($changefreq);
                    }
                    if (isset($url['priority'])){
                        $priority = $row->ownerDocument->createElement('priority', $url['priority']);
                        $row->appendChild($priority);
                    }
                    $new = $dom->insertBefore($row, $dom->firstChild);
                    simplexml_import_dom($new, get_class($xml));
//                    $row->addChild('loc', htmlspecialchars($url['loc'], ENT_QUOTES, 'UTF-8'));
//                    if (isset($url['lastmod']))
//                        $row->addChild('lastmod', $url['lastmod']);
//                    if (isset($url['changefreq']))
...
//                    if (isset($url['priority']))
//                        $row->addChild('priority', $url['priority']);
                    //$this->simplexml_insert_before($row, $target);
                }
            }
