...
class PaginationWap{
	...
	public function paging($count,$per_page,$cur_page,$url='')
	{
           $paging = '';
            if (($count / $per_page) > 1) {
                $previous_btn = true;
                ...
                if ($first_btn && $cur_page > 1) {
                    $paging .= "<span p='1' class='inactive first'><a href='" . base_url() . $url . "?trang=1'>&nbsp;</a></span>";
                } else if ($first_btn) {
                    $paging .= "<span p='1' class='inactive first'><a href='" . base_url() . $url . "?trang=1'>&nbsp;</a></span>";
                }
                ...
                if ($previous_btn && $cur_page > 1) {
                    $pre = $cur_page - 1;
                    //$paging .= "<li p='" . $pre . "' class='inactive pre'><a href='" . base_url() . "?trang=" . $pre . "'>&nbsp;</a></li>";
                } else if ($previous_btn) {
                    //$paging .= "<li class='pre'><a href='javascript:void(0)'>&nbsp;</a></li>";
                }
                $curp = 1;
                for ($i = $start_loop; $i <= $end_loop; $i++) {
                    if ($cur_page == $i) {
                        ...
                        $paging .= "<span p='$i' class='inactive'><a href='" . base_url() . $url . "?trang=" . $i . "'>" . $i . "</a></span>";
                    }
                }
                ...
                if ($last_btn && $cur_page < $no_of_paginations) {
                    $paging .= "<span p='" . $no_of_paginations . "' class='inactive last'><a href='" .  base_url() . $url . "?trang=" . $no_of_paginations . "'>&nbsp;</a></span>";
                } else if ($last_btn) {
                    $paging .= "<span p='$no_of_paginations' class='inactive last'><a href='" . base_url() . $url . "?trang=" . $no_of_paginations . "'>&nbsp;</a></span>";
                }
                $paging .= "</div>";
