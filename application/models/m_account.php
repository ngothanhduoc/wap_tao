...
class M_account extends MY_Model {
    ...
    public function get_by_username($username){
        $sql = $this->db_slave->select('u.*, user_group.permission, user_group.name')
			      ->from('user u')
                              ->join('user_group', 'u.id_group = user_group.id_group')
    ...
    public function get_menu_by_userid($id=0){
     ...
     public function get_menu_by_group($arrId=array()){
        $sql = $this->db_slave->select('m.name_display,m.url,mg.display_name as group_name,mg.class,m.alias as als,mg.alias')
                ->from('function m')
                ->join('function_group mg', 'm.parent_id = mg.id')
                ->where_in('m.id_function',$arrId)
                ->where('m.is_leaf', 1)
		->where('mg.is_display', 1)
                ->order_by('mg.order')
                ->get();
